const btnElm=$('#btn-add');
const nameElm=$('#txt-name');
const addressElm=$('#txt-address');
const contactElm=$('#txt-contact');
const tableBElm=$('#tbl-customers tbody');
const API_BASE_URL='http://localhost:8080/pos';

[nameElm,addressElm,contactElm].forEach(elm=>{
        elm.addClass('animate__animated');

})

loadCustomers();

function loadCustomers(){
        const xhr=new XMLHttpRequest();

        xhr.addEventListener('readystatechange',()=>{
                if(xhr.readyState==4){
                        if(xhr.status==200){
                            let customerList  = JSON.parse(xhr.responseText);
                            customerList.forEach(elm=>{
                                    tableBElm.append(`<tr>
                <td></td>
                <td>${elm.name}</td>
                <td>${elm.address}</td>
                <td>${elm.contact}</td>
                <td class="d-flex justify-content-between">
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                        </svg>

                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                            <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5Zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6Z"/>
                            <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1ZM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118ZM2.5 3h11V2h-11v1Z"/>
                        </svg>
                    </div>
                </td>
            </tr>`) })
                        }
                }
        });

        xhr.open('get',API_BASE_URL+'/v1/api/customers',true);

        xhr.send();


}

btnElm.on('click',()=>{
        if(!isValid()) return;

        const name=nameElm.val().trim();
        const address=addressElm.val().trim();
        const contact=contactElm.val().trim();

        let customer={
                name,address,contact
        }

        const xhr=new XMLHttpRequest();

        xhr.addEventListener('readystatechange',()=>{
               if(xhr.readyState==4){
                       if(xhr.status==200){
                               console.log(xhr.responseText);
                       }
               }
        })

        xhr.open('post',API_BASE_URL+'/v1/api/customers',true);

        xhr.setRequestHeader('content-type','application/json');

        xhr.send(JSON.stringify(customer));


})

function isValid(){
        const name=nameElm.val().trim();
        const address=addressElm.val().trim();
        const contact=contactElm.val().trim();

        let valid=true;
        [nameElm,addressElm,contactElm].forEach(elm=>{
                elm.removeClass(['is-invalid','animate__shakeX']);

        })

        if(!contact){
                valid=inValidate(contactElm,'Contact cant be empty');
        }else if(!(/^\d{3}-\d{7}$/.test(contact))){
                valid=inValidate(contactElm,'Invalid Contact');

        }

        if(!address){
                valid=inValidate(addressElm,'Address cant be empty');
        }else if(!(/^.{3,}$/.test(address))){
                valid=inValidate(addressElm,'Invalid Address');

        }

        if(!name){
                valid=inValidate(nameElm,'name cant be empty');
        }else if(!(/^[a-zA-Z ]{3,}$/.test(name))){
                valid=inValidate(nameElm,'Invalid name');

        }

}

function inValidate(elm, txt) {
        elm.addClass(['is-invalid','animate__shakeX']);
        elm.next().text(txt);
        elm.trigger('select');
        return false;
}
